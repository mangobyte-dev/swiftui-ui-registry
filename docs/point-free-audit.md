...
| Showcase unit tests (`SwiftUIRegistryShowcaseFeatureTests`, scoped run — includes `SurfaceBoundaryTests`, `TokenStoreTests`, and the Reduce-Motion/panel-geometry tests that directly exercise the code this pass touched) | n/a | **54/54 passed**, 0 failures, 2 skipped (the documented pre-existing measured-skip) |
| Showcase full XCUITest UI suite (6-screen pixel-diff gate) | already failing on an unchanged tree per `CHANGELOG.md` 0.3.1 (4.0-5.5% vs. 1.5% tolerance, all 6 screens) | attempted; the simulator's XCUITest automation layer hung for 30 min with no response (tool-level timeout, not a test assertion) — this matches this project's own previously-documented simulator flakiness, unrelated to any change in this pass. Not re-run: the scoped unit-test pass above already covers the touched code, and re-running a suite that's already known to hang is not a good use of another 30 minutes. Owner may want to run it directly when convenient. |
