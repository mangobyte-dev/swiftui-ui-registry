...
- Three visual references are stale after the audit and `auth-light.png` fails at 1.95 percent: the approved image predates the wrapping code block (both landed in e2a75ef) and the input fields are now 44 points tall. Replacing `auth-light.png`, `activity-light.png`, and `nutrition-light.png` from the reviewed audit attachments with a GOLDEN-CHANGE note is the owner's action (the Audit section names the commands); the other 17 UI tests pass
