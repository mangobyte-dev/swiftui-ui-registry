...
1. Replace the two visual references from the final suite run's kept attachments, reviewed first, then re-run the suite. The attachments are exported next to this session's result bundle; the copy commands are listed under "Reference replacement" below. Until then `auth-light` (2.67 percent, stale since the audit) and `nutrition-light` (1.53 percent) fail their visual assertion because those blocks reach the accent strip; the other 17 UI tests and 7 Showcase unit tests pass
...
```sh
OUT=<exported attachments folder>   # xcrun xcresulttool export attachments --path <final-suite.xcresult> --output-path "$OUT"
